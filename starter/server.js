...
//
// STARTER: one spot is left as TODO -- see exercises/part-1-echo-stream.md (Task B).
...
async function handleEcho() {
  ...
  while (true) {
    ...
    while (true) {
      ...
      // TODO (Task B): read from `reader` in a loop and write each received value
      // back unchanged via writer.write(value). Stop when `done` is true.
      // Reference: solution/server.js
      (async () => {
        try {
          // ... your code here ...
        } catch (err) {
...
// The server side is given; the client is your task (see exercises/bonus-datagrams.md).
